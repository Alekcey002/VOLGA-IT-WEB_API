...
namespace Document.Servers
{
    public class AuthenticationService
    {
        ...
        public async Task<List<User>> GetPacientAsync(string token)
        {
            using (var httpClient = new HttpClient())
            {
                httpClient.BaseAddress = new Uri("http://localhost:8001/api/");
                httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
                httpClient.DefaultRequestHeaders.Add("Origin", "http://localhost:8002");

                var response = await httpClient.GetAsync("Accounts");
                if (response.IsSuccessStatusCode)
                {
                    var content = await response.Content.ReadAsStringAsync();
                    var responseObject = JsonConvert.DeserializeObject<List<User>>(content);
